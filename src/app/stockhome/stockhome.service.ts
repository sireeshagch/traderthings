...
export class StockHomeService {
  ...
  getNasdaq() {
   return this.http.get('https://cheesepy.com/stockprice/TIME_SERIES_DAILY/1/%5EIXIC')
    .map(res => res.json());
  }

  getSnP() {
    return this.http.get('https://cheesepy.com/stockprice/TIME_SERIES_DAILY/1/%5EGSPC')
     .map(res => res.json());
   }

   getDow() {
    return this.http.get('https://cheesepy.com/stockprice/TIME_SERIES_DAILY/1/%5EDJI')
     .map(res => res.json());
   }
